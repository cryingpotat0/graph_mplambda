...
namespace mpl::demo {
    ...
    decltype(auto) findPathsFromStartToGoals(const Graph& graph, Scenario& scenario, const Vertex& start, AppOptions& app_options) {
        using State = typename Scenario::State;
	using VertexID = typename Vertex::ID;
	    ...
	    //graph.djikstras(start.id(), start.id());
	    //graph.djikstras(start.id(), scenario);
	    ...
	    JI_LOG(INFO) << "Time to do djikstras for start " << start.state() << " goal " << goal
		    << " is " << std::chrono::duration_cast<std::chrono::milliseconds>(end_djikstras - start_djikstras);;
	    auto identifier = std::make_pair(start.state(), goal);
	...
	auto paths = findPathsFromStartToGoals(coord.getGraph(), scenario, coord.getStartVertex(start), app_options);
        saveSolutionPaths<Coordinator, State>(coord, app_options, paths);
    }
    
    template <class Coordinator, class Scenario>
    void fetchPostProcessing() {
    }
}
