    ...
    template <class Scenario, class Scalar>
        class CoordinatorFixedGraph {
            ...
            private:
                ...
                std::uint64_t global_num_uniform_samples{0};
                ...
                void init_local_lambdas() {
                    ...
                    void update_num_samples(std::uint64_t lambdaId, size_t successful_samples) {
                        ...
                        if ((*curr_min_samples) > global_min_samples) {
                            ...
                            global_num_uniform_samples = global_min_samples * app_options.jobs();
                            JI_LOG(INFO) << "Updating global_num_samples to " << global_num_uniform_samples;
                            for (auto& c: connections_) {
                                c.write(packet::NumSamples(global_num_uniform_samples));
                            }
                        } else {
                    ...
                    const std::uint64_t getGlobalNumUniformSamples() const {
                        return global_num_uniform_samples;
                    }
                    ...
                    void loadGraph(std::string filename) {
                        ...
                        auto pos = line.find(":");
                        global_num_uniform_samples = std::stoull(line.substr(pos + 1));
