...
namespace mpl {
    ...
    template <class Scenario, class Scalar> class CoordinatorFixedGraph {
        public:
        ...
        private:
            ...
            std::vector<std::pair<std::uint64_t, std::uint64_t>>
                timed_global_num_uniform_samples_;
            ...
            void init_aws_lambdas() {
                ...
                for (int i = 0; i < app_options.jobs(); ++i) {
                    ...
                    std::vector<std::string> args = {
                        "scenario", app_options.scenario(), "global_min",
                        app_options.global_min_, "global_max", app_options.global_max_,
                        "lambda_id", lambdaId, "algorithm", app_options.algorithm(),
                        "coordinator", app_options.coordinator(),
                        //"communicator", app_options.communicator(),
                        "num_samples", std::to_string(app_options.numSamples()), "time-limit",
                        std::to_string(app_options.timeLimit()), "env",
                        app_options.env(false), "env-frame", app_options.envFrame_,
                        "num_divisions", app_options.num_divisions_, "graph-size",
                        std::to_string(app_options.graphSize())};
            ...
            void init_local_lambdas() {
                for (int i = 0; i < app_options.jobs(); ++i) {
                    ...
                    std::vector<std::string> args = {
                        ...
                        "--time-limit", std::to_string(app_options.timeLimit()), "--env",
                        app_options.env(false), "--env-frame", app_options.envFrame_,
                        "--num_divisions", app_options.num_divisions_, "--graph-size",
                        std::to_string(app_options.graphSize())};
                    for (int i = 0; i < app_options.starts_.size(); ++i) {
                    ...
                    JI_LOG(TRACE) << "Running lambda" << lambdaId << " file " << file << ": "
                        << command;
        ...
        public:
            CoordinatorFixedGraph(demo::AppOptions &app_options_)
                ...
                global_subspace(Subspace_t(app_options_.globalMin<Bound>(),
                            app_options_.globalMax<Bound>())) {
#if HAS_AWS_SDK
            ...
            inline std::string algorithm() { return "prm_fixed_graph"; }
            ...
            void update_num_samples(std::uint64_t lambdaId, size_t successful_samples) {
                num_samples_per_lambda_[lambdaId] +=
                    app_options
                    .numSamples(); // Regardless of how many successful samples, each
                // lambda tries to sample these many points
                auto curr_min_samples = std::min_element(num_samples_per_lambda_.begin(),
                ...
                if ((*curr_min_samples) > global_min_samples) {
                    ...
                    // JI_LOG(INFO) << "Updating global_num_samples to " <<
                    // global_num_uniform_samples_;
                    auto stop = std::chrono::high_resolution_clock::now();
                    std::uint64_t curr_time =
                        std::chrono::duration_cast<std::chrono::milliseconds>(stop -
                                start_time)
                        .count();
                    timed_global_num_uniform_samples_.push_back(
                            std::make_pair(curr_time, global_num_uniform_samples_));
                    for (auto &c : connections_) {
                        c.write(packet::NumSamples(global_num_uniform_samples_));
                    }
                } else {
                    // JI_LOG(INFO) << "Not updating global_num_samples " <<
                    // global_min_samples << " from curr_min " << *curr_min_samples;
                }
                // JI_LOG(INFO) << num_samples_per_lambda_;
            }

            const TimedGraph &getGraph() const { return graph; };

            TimedGraph &getGraph() { return graph; };
            ...
            //                JI_LOG(ERROR) << "BIG ERROR, VERTICES SHOULD BE ADDED BEFORE
            //                EDGES";
            //            }
            //            if (curr_edge.timestamp_millis() > time_limit) continue;
            //            return_graph.addEdge(Edge{curr_edge.distance(), curr_edge.u(),
            //            curr_edge.v()});
            //        }
            //    }
            //    return return_graph;
            ...
            void start_socket() {
                ...
                if (::getsockname(listen_, reinterpret_cast<struct sockaddr *>(&addr),
                            &addrLen) == -1)
            ...
            void divide_work() {
                ...
                std::vector<int> num_divisions = std::vector<int>(
                        ...
                        eig_num_divisions.data() +
                        eig_num_divisions.rows() * eig_num_divisions.cols());
            ...
            void loop() {
                ...
                for (;;) {
                    ...
                    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                            stop - start_time);
                    if (duration.count() > app_options.timeLimit() * 1000 ||
                            connections_.size() ==
                            0) { // specified time limit in s or exit when all lambdas exit,
                        // i.e. when the graph size limit has been reached
                        if (!done_)
                            done_ = 1;
                    }
                    ...
                    auto pit = pfds.begin();
                    for (auto cit = childProcesses_.begin(); cit != childProcesses_.end();
                            ++pit) {
                        assert(pit != pfds.end());
                        ...
                        if ((pit->revents & POLLHUP) == 0) {
                            ...
                            JI_LOG(INFO) << "child process " << cit->first
            ...
            const std::uint64_t
                getGlobalNumUniformSamples(std::uint64_t time_limit) const {
                    std::uint64_t prev = 0;
                    for (auto &[time, global_num_uniform_samples_curr] :
                            timed_global_num_uniform_samples_) {
                        if (time > time_limit)
                            break;
            ...
            void saveGraph(std::string filename) {
                ...
                for (auto &[time, global_num_uniform_samples_curr] :
                        timed_global_num_uniform_samples_) {
                    file << "num_global_uniform_samples;" << time << ";"
            ...
            void loadGraph(std::string filename) {
                ...
                for (std::string line; std::getline(file, line);) {
                    if (line == "end_num_global_uniform_samples")
                        break;
                    std::vector<std::string> results;
                    mpl::util::string_split(results, line, ";");
                    timed_global_num_uniform_samples_.push_back(
                            std::make_pair(std::stoull(results[1]), std::stoull(results[2])));
    ...
    template <class Scenario, class Scalar> class CoordinatorCommonSeed {
        public:
            using State = typename Scenario::State;
            using Bound = typename Scenario::Bound;
            using Distance = typename Scenario::Distance;
            ...
            void init_aws_lambdas() {
                ...
                for (int i = 0; i < app_options.jobs(); ++i) {
                    ...
                    std::vector<std::string> args = {
                        ...
                        "num_samples", std::to_string(app_options.numSamples()), "time-limit",
                        std::to_string(app_options.timeLimit()), "env",
                        app_options.env(false), "env-frame", app_options.envFrame_, "jobs",
                        std::to_string(app_options.jobs_), "random_seed",
                        std::to_string(app_options.randomSeed_), "graph-size",
                        std::to_string(app_options.graphSize())};
                    args.push_back("start");
                    std::string starts;
                    for (int i = 0; i < app_options.starts_.size(); ++i) {
        ...
        public:
            ...
            inline std::string algorithm() { return "prm_common_seed"; }

            void addVertices(const std::vector<Vertex> &vertices) {
                // auto stop = std::chrono::high_resolution_clock::now();
                // std::uint64_t curr_time =
                // std::chrono::duration_cast<std::chrono::milliseconds>(stop -
                // start_time).count(); for (auto& v : vertices) {
                //    //JI_LOG(INFO) << "Adding " << v.id() << " at " << curr_time;
                //    graph.addVertex(TimedVertex{v.id(), v.state(), curr_time});
                //}
                // Vertex adding moved to addEdges method to prevent overhead of sending
                // vertices
            }
            ...
            void addEdges(const std::vector<Edge> &edges) {
                ...
                for (auto &e : edges) {
                    // JI_LOG(INFO) << "Adding " << e.u() << "-" << e.v() << " at " <<
                    // curr_time;
                    graph.addVertex(TimedVertex{
                            e.u(), s /* proxy state, updated in postprocessing */, curr_time});
                    graph.addEdge(TimedEdge{e.distance(), e.u(), e.v(), curr_time});
                    if (e.u().second > max_vertex_id_)
                        max_vertex_id_ = e.u().second;
                }
            }
            ...
            void loop() {
                ...
                for (;;) {
                    ...
                    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                            stop - start_time);
                    if (duration.count() >
                            app_options.timeLimit() * 1000) { // specified time limit in s
                        if (!done_)
                            done_ = 1;
                    }
            ...
            const std::vector<Subspace_t> getSubspaces() const {
                ...
                subspaces.push_back(Subspace_t(app_options.globalMin<Bound>(),
                            app_options.globalMax<Bound>()));
                return subspaces;
            }
    };
